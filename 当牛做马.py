...
class NiumaCalculator:
    ...
    def update_display(self):
        ...
        if is_today_workday:
            ...
            if current_time >= morning_start:
                if current_time <= morning_end:
                    ...
                    morning_earned = worked_seconds * secondly_wage
                    is_working = True
                    status_text = "上午搬砖中... 💪"
                else:
                    # 上午已结束，拿满上午工资
                    morning_duration_seconds = (
                            datetime.datetime.combine(now.date(), morning_end) -
                            datetime.datetime.combine(now.date(), morning_start)
                    ).total_seconds()
                    morning_earned = morning_duration_seconds * secondly_wage

            # 2. 计算下午已赚
            afternoon_earned = 0.0
            if current_time >= afternoon_start:
                if current_time <= afternoon_end:
                    ...
                    afternoon_earned = afternoon_worked_seconds * secondly_wage
                    is_working = True
                    status_text = "下午搬砖中... 💪"
                else:
                    # 下午已结束，拿满下午工资
                    afternoon_duration_seconds = (
                            datetime.datetime.combine(now.date(), afternoon_end) -
                            datetime.datetime.combine(now.date(), afternoon_start)
                    ).total_seconds()
                    afternoon_earned = afternoon_duration_seconds * secondly_wage

            # 3. 汇总今日收入
            earned_today = morning_earned + afternoon_earned
        ...
        # 如果今天是工作日且已过下班时间，或者今天是节假日，今日贡献为0或全额日薪？
        # 通常逻辑：过去完整工作日的钱 + 今天实时赚的钱
